...
Order updated 2026-08-30 by owner ruling: N2c closes the NOW block, then gate 2, then M2.0 starting at T1. Decision 1 is no longer ahead of the gate — it is M2.0's T3, where `m2-0-tasks.md` designed it and where its predecessors sit.
...
- [x] **N2 Video surface show and re-show (decision 2).** Build the re-show path for the native surface with a video already loaded. Prerequisite of M2.0's occlusion handling (decision 1).
  ...
  - **Correction, gate 2 (2026-08-30):** the automated suite does not assert the visible frame. `062f201` rewrote `video-surface.spec.js` and replaced the pixel/saturation check this AC describes with map-state and `childWindows().length > 0` checks; the file's own header says so ("What is asserted here is the surface coming back mapped with mpv still attached to it — not the pixels"). That change was never named in `062f201`'s delivery description, as WORKFLOW §4 requires for a weakened assertion. The reasoning in this AC still holds — it is why the suite checks mpv's mapped child window instead of an internal flag — but the pixel half of "the frame is visible" is verified by hand only, three launches on the owner's own hardware (`docs/reports/n2b-collaudo-reale.md`), not by anything CI runs.
...
- [x] **N2c Video surface is misplaced under fractional scaling (fixed 2026-08-30).** On the owner's display — 3840x2160 at scale 1.5 — the video plays but lands at a fraction of its rectangle, over the transcription bar instead of the stage.
  ...
  - **Correction, gate 2 (2026-08-30):** the first AC's screenshot evidence (`docs/reports/n2c-p3-scala.md:73`, "a capture of the window shows the picture inside the stage") names no capture command and no image is committed to the repo. The measured half of that AC — the surface geometry, 592x180 at 432,500 — is independently re-checkable and was re-derived from the page's own formula during gate 2; only the picture half rests on an unrepeatable sentence.
...
- [x] **N1b The window segfaults on exit after the close gate saves, under load (fixed 2026-08-30).** The main thread dies inside `_gdk_x11_display_queue_events` one loop iteration after `close_window` asked for `window.destroy()`. Two hypotheses have been measured and killed: rfd's second GTK thread (the crash survives its removal, with a core showing one GTK thread and no rfd frame) and "the sequential rate is high" (sixty sequential runs, thirty per branch, produced nothing at all).
  ...
  - **Correction, gate 2 (2026-08-30):** this AC was not met by the evidence the checkbox below certifies. The delivered binary was run three times sequentially, not thirty (`docs/reports/n1b-sessanta-corse.md:70`, "sequential `pnpm e2e:close-gate` | 30 green, 0 red | 3 green, 0 red" — the thirty is the pre-fix binary, the three is the one shipped), and the gap was never closed by running the remaining twenty-seven or by lowering this line to match. The number to trust for the delivered binary is three, not thirty.
...
- [ ] **N4 Launching without a display panics instead of explaining (minor, filed 2026-08-30).** `sublore` with no `DISPLAY` and no `WAYLAND_DISPLAY` — over SSH, in a container, on a display that went away — dies with a raw library panic from `tao-0.35.3/src/platform_impl/linux/event_loop.rs:217`, `Failed to initialize gtk backend!`, plus a `RUST_BACKTRACE` note. Intelligible to a developer and useless to anyone else, and CLAUDE.md section 6 asks for actionable messages at boundaries. Pre-existing, not introduced by the gate 2 range, and found while reproducing a harness artefact rather than by looking for it.
...
- [ ] **N5 Residue of gate 2, filed rather than fixed (2026-08-30).** Every row here comes from the gate's own register or from Wave 4, and each is a minor by the owner's ruling that the gate opens on blockers and serious, not on minor perfection. Grouped because they share owners and would otherwise reopen the same files one at a time.
  - `player.rs:66` — `SUBLORE_MPV_GPU_CONTEXT=` (set, empty) means "leave mpv alone", while `SUBLORE_WEBKIT_WORKAROUNDS=` (set, empty) falls through to the probe. Two hatches, same prefix, opposite readings of the same input. `main.rs` settled the rule; `player.rs` has not followed it.
  - `player.rs:209` — the hatch is reported through `std::env::var(..).ok()`, so a value that is not UTF-8 prints as "unset" for a variable that is set. The same `OsString` lesson `startup_files` was rewritten to learn in the same range, not applied beside it.
  - `player.rs` — an unrecognised `gpu-context` value is forwarded verbatim, mpv's initializer rejects it, and the user gets **no video at all** rather than a fallback and a warning. A typo costs playback.
  ...
  - `e2e/lib/pixels.js` — `saturation()` now has no caller anywhere; `requireFfmpeg()` still has one. Dead code by CLAUDE.md section 6. `e2e/README.md:7-10` still explains ffmpeg as the harness's pixel instrument, which is no longer why it is needed.
  - `VideoStage.tsx` — the `matchMedia` listeners added for a live device-ratio change are fired by nothing. Exercising them needs a frontend test runner with a fake `matchMedia`, which is a new dependency and an owner decision (WORKFLOW section 3), or an XSETTINGS rig that flips the scaling factor under a running app.
  - Correction owed to `docs/reports/gate2-closure-audit.md`: its row for `e2e/wdio.conf.js:34` says the ffmpeg gate guards "a dependency nothing needs". A run with ffmpeg hidden contradicts that. Fix the row so nobody deletes the gate on its authority.
- [ ] **N6 `ExitRequested` never prevents the exit (latent data-loss path, filed 2026-08-30).** The close gate protects `CloseRequested` and nothing else. `RunEvent::ExitRequested` has no `prevent_exit`, which is safe today only because nothing calls `AppHandle::exit`. The moment a menu item, a tray entry or a keyboard shortcut quits the app — and M2.0's T7 builds a menu bar — the gate is bypassed and unsaved work leaves with the process. Found by Wave 4 while reading something else.
...
- [ ] **M2.0 Shell redesign — blocks M2.4-M2.6.**
 ...
 Filed 2026-08-29 after the owner ran the app and rejected the interface. M0-M4 each bolted a horizontal band with a path field and a button onto one column, because that is the cheapest way to give an E2E spec a stable selector; the result is the union of five test harnesses. Rebuild the shell on Aegisub's structure with a modern finish, per `docs/design/shell-layout.md` and `docs/design/shell-mockup.html`. The engine is not touched: this is the frontend shell plus the file-dialog commands.
  ...
  - AC: the existing E2E checks pass with selectors re-pointed and assertions unchanged — the count is whatever `EXPECTED_TESTS` in `e2e/wdio.conf.js` says on the day, 33 at 2026-08-30, and it never decreases; no assertion is weakened, skipped, or retargeted (CLAUDE.md §5.4).
