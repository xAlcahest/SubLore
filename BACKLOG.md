...
- [ ] **N2c Video surface is misplaced under fractional scaling (product defect, filed 2026-08-30).** Found during the N2b real-session check. On the owner's display — 3840x2160 at scale 1.5 — the video plays but lands at 395x120 over the transcription bar instead of covering the stage, overlapping the controls. `VideoStage` reports a rectangle in CSS pixels and `apply_region` multiplies it by `window.scale_factor()`; at a fractional factor the result is wrong in both position and size. Runs after N2b and before decision 1.
