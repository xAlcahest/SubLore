...
- [ ] **N2c Video surface is misplaced under fractional scaling (product defect, filed 2026-08-30).** Found during the N2b real-session check. On the owner's display — 3840x2160 at scale 1.5 — the video plays but lands at 395x120 over the transcription bar instead of covering the stage, overlapping the controls.
  - **The mechanism is unknown.** The first explanation, that `apply_region` multiplies by `window.scale_factor()` and mishandles a fractional factor, was wrong and is withdrawn: `surface/linux.rs:63-64` uses `logical()` and never applies the scale factor, because GDK applies it itself. `physical()` is the Windows path. Any fix written before the mechanism is known is a guess.
  - AC: probe P3 of `docs/design/x11-vs-render-api.md` runs first — read `window.devicePixelRatio` in the webview and `window.scale_factor()` from Rust on that display and write down both. If both report 1, the fractional factor arrives from somewhere else entirely; if they disagree, the disagreement is the bug. The result goes in a report before any code is written.
  - AC: at scale 1.5 the surface covers the stage. Verified on the real display with a screenshot of the app's own window, since a scaled display is exactly what the harness cannot reproduce.
  - AC: whatever the conversion turns out to be, it is pinned by a unit test that can run in CI, covering the fractional case and the 16-bit X11 clamp.
- [ ] **N1b The window sometimes segfaults on exit after the close gate saves (product defect, filed 2026-08-30).** Found during the N2b battery. `close-gate-check.js` fails its last checks with `{"code":null,"signal":"SIGSEGV"}`: 1 run in 12, and 2 in roughly 17 across the afternoon, always on the save branch. The file is written and the backup kept before the crash, so no data was lost in what was observed. The core dump puts the crashing frame in `_gdk_x11_display_queue_events` inside `gtk_main_iteration_do`, with no Sublore frame above `run()`: GTK is still pulling X events from a display in teardown, one iteration after `close_window` asked for `window.destroy()`. Full backtrace in `docs/reports/n1b-segfault-uscita.md`. **One attempt has failed:** the close gate's dialogs were moved off rfd onto GTK on the main thread, which removes the second GTK thread from the process, and the crash survived it — 25 green, 1 SIGSEGV and 4 unexplained non-crash failures over 30 runs (`n1b-trenta-corse.md`), with a core from the new binary showing the same crashing frame and no rfd thread anywhere. The rfd hypothesis is refuted. Until this is fixed the close gate check is red in CI roughly once in a dozen runs; the check stays as it is, because it is catching a real crash.
  - AC: 30 consecutive runs of `pnpm e2e:close-gate` with no SIGSEGV and no other non-zero exit. 30 because at the observed rate a shorter run proves nothing.
  - AC: the fix does not weaken any assertion in the check, and the save branch still proves the file was written and the backup kept.
