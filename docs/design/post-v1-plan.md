...
Produced by a 23-agent survey of the subtitle-editing command space (251 candidate commands, 8 domains, every one accounted for), each feature then weighed against Sublore's own code, three adversarial passes, and a synthesis.
...
**Acceptance criteria** (observable behaviour, per CONTRIBUTING.md §5)
...
- Sulla fixture da 2000 cue, Ctrl+A seguito da freccia giu non blocca l'interfaccia oltre il budget di CONTRIBUTING.md sezione 7, misurato e riportato.
...
- Il motore vive in un crate aperto e condiviso, non dentro il modulo chiuso M5. CONTRIBUTING.md sezione 4 vuole il core aperto pienamente utile da solo, e il QA termbase ha bisogno esattamente dello stesso confronto.
...
- Sfoglia recuperi: elenco delle copie di emergenza per file, con orario, e apertura di quella scelta. Cancellare una copia resta un gesto dell'utente, mai una pulizia automatica (CONTRIBUTING.md sezione 3.3).
...
- Secondo gradino, opzionale e da decidere col proprietario: motore ortografico lato Rust con scelta della lingua e tokenizzazione che salta i tag override. Richiede una dipendenza nuova compatibile GPL e, soprattutto, una decisione sui dizionari, perche CONTRIBUTING.md sezione 1 elenca le uniche eccezioni di rete ammesse e i dizionari non sono fra quelle.
...
- Import della traccia sottotitoli da un contenitore MKV, estraendola con ffmpeg che e gia strumento obbligatorio scoperto all'avvio (crates/sublore-asr/src/tools.rs:36-45) verso la cartella di lavoro, senza mai toccare il file video (CONTRIBUTING.md sezione 3.1). Serve aggiungere l'enumerazione delle tracce, oggi assente perche ffprobe non e nella scoperta.
...
- Gestore stili e catalogo a livello di serie nel database di progetto, che ha gia il runner di migrazioni versionato. Non in un file di preferenze per utente: la serie e la nostra unita naturale.
...
- Cosa si rischia: i budget di CONTRIBUTING.md sezione 7 (avvio sotto due secondi, memoria a riposo sotto 400 MB). Un giro di texture per fotogramma dentro un webview e il modo classico di sfondarli, e il risultato va misurato prima di considerare fatta la milestone, non dopo.
...
- Il video si vede dentro la finestra come prima, e i controlli E2E esistenti passano con le asserzioni invariate, nessuna riscritta al ribasso e nessuna saltata (CONTRIBUTING.md sezione 5.4).
...
- Gli stessi controlli passano su Windows e su Linux, entrambi in CI (CONTRIBUTING.md sezione 5.5).
...
- **Cost if deferred:** Una regressione di sicurezza dei dati contro CONTRIBUTING.md sezione 3.3, scoperta dall'utente nel momento in cui gli serve un backup e non c'e piu. E il tipo di bug che il progetto esiste per non avere.

### 9. Il gate delle modifiche non salvate alla chiusura della finestra. Oggi la gestione di CloseRequested spegne solo trascrizione e video (src-tauri/src/lib.rs) e non esiste alcun prevent_close nel repo; lo stato sporco e tracciato sia lato backend sia lato frontend, e nessuno lo consulta alla chiusura. Chiudere la finestra con edit non salvati li butta via in silenzio, contro CONTRIBUTING.md sezione 3, che dice che un bug puo costare fastidio e mai dati. Il plugin di dialogo e gia una dipendenza.
...
- **Comandi specifici di macOS e finestre multiple di sistema, e l'aggiornamento automatico.** — macOS e differito per decisione del proprietario, e l'auto-updater e in parcheggio esplicito (BACKLOG.md:129). Il controllo aggiornamenti esplicito da menu resta ammesso da CONTRIBUTING.md sezione 1, ma richiede un canale di rilascio pubblico che oggi non esiste: quando ci sara, costa poche ore.
- **Catena di filtri configurabile all'export.** — L'idea buona, cioe l'export come pipeline dichiarata e riproducibile, appartiene al modulo batch, che e chiuso per CONTRIBUTING.md sezione 4 e in parcheggio per BACKLOG.md:129. Costruirla nel core aperto significherebbe costruire l'impalcatura di una funzione che vivra altrove. La conversione semplice fra i nostri tre formati, che e cio che serve davvero, sta in M13.
