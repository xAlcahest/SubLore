...
- **Synthetic input belongs in an isolated server.** `xdotool` typing, clicks and key presses are allowed only inside an X server the harness owns and started itself (Xvfb). They are never used on the owner's real display: on a live compositor keystrokes go to whichever window holds the focus, and during the N2b check they landed in the owner's own window and typed a fixture path into it. **The harness now refuses instead of trusting whoever runs it:** every `xdotool` call goes through a check for a window manager on `$DISPLAY`, and a display that has one is a real session, not a test server (`e2e/lib/input.js`). The rule was written on 2026-08-30 and broken on 2026-08-31 by the person who wrote it, running `pnpm e2e:close-gate` without `xvfb-run`; a rule that only lives in a document is a rule that gets broken while nothing notices.
...
- **The last merge in a wave re-checks the file's citations.** When two implementers touch one file in the same wave, line numbers move under whoever wrote first, and a citation that was correct when it was written stops resolving. This has reopened a closed register row twice — `decisions.md` pointing into `video/surface/`, both times. Whoever lands last greps the documents for that file and fixes what no longer resolves.
