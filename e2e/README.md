...
| `scripts/quit-gate-check.js`            | 17 checks                                                                            | A quit that is not a window close — `AppHandle::exit`, what a menu's Quit item and Ctrl+Q call — asks the same question the X button asks: it raises the unsaved-changes dialog, cancel keeps the app and the file, a second quit asks again instead of riding the cancelled answer out, discard exits 0 leaving the file untouched, and save writes the edit and moves nothing else (BACKLOG N6). The route is driven through the debug-only `SUBLORE_QUIT_ON_FILE` hook, and the check fails unless the app's log says the quit really went that way. |
