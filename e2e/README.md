...
| `scripts/picker-thread-check.js`        | 9 checks                                                                             | Choosing a project folder and a project file leaves no thread but the main one running `gtk_main_iteration`, read with `eu-stack`, and a cancelled choice still returns as a cancellation (BACKLOG N1c).                                                                                                                                                                                                      |
...
xvfb-run -a -s "-screen 0 1280x1024x24" pnpm e2e           # the nine WebDriver spec files
...
xvfb-run -a -s "-screen 0 1280x1024x24" pnpm e2e:mpv-context  # a refused gpu-context costs the request, not the window
```
...
`.subbar__open`, `.subbar__save`, `.subbar__status`, `.subbar__error`,
...
extended for that: it is shared with every spec and stays frozen.
