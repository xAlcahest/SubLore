...
- **Full Windows activation is its own mandatory milestone**, covering the E2E backend with native input and window inspection, platform hardening, and the owner checklist run on Windows.
- **Owner ruling 2026-09-04, superseding the sentence that stood here.** v1.0 releases on Linux alone. Windows keeps compiling in CI on every push and nothing may be introduced that would block it, but the behavioural suite stays Linux-only and **no Windows behaviour is claimed anywhere**: not in the app, not in the README, not on a store page. MW stays open and stays mandatory before a Windows build ships to anyone; it no longer gates the Linux release.
- The reason is written down rather than left implied. MW.1b has to be written by running it, there is no Windows machine a coding agent can reach, and MW.3 is a checklist only the owner can sign. **A Windows binary can be loaded and traced here, which is not the same thing and is worth knowing:** wine plus the `x86_64-pc-windows-gnu` target plus the pinned libmpv's own `.dll.a` will link and start a Windows build, and `WINEDEBUG=+loaddll` prints the loader's decisions. That found the `WebView2Loader.dll` the test binaries were missing (BACKLOG.md N29) in one run, where the CI diagnostic had named the wrong symbol twice. It proves loading and linking; it is not a behavioural verdict and nothing run under it may be reported as Windows behaviour. Holding the Linux release behind those would price the release in owner hours rather than in work.
...
5. **Behavioural verification happens on Linux; Windows compiles.** The E2E suite drives the app through X11 and runs on Linux only, so today it proves Linux behaviour and nothing else. The Windows `check` job must stay green on every push, and a green compile is never reported as a working feature. The full matrix — behavioural suite green on Windows too — is the exit condition of the Windows activation milestone, which under the 2026-09-04 ruling gates a Windows build and not the Linux release. macOS joins when the owner re-activates it. "Works on my platform" does not exist here; neither does "it compiled". Check the Windows build before pushing rather than after: `rustup target add x86_64-pc-windows-gnu` once, then `cargo clippy --target x86_64-pc-windows-gnu -p sublore --all-targets -- -D warnings` compiles the `cfg(windows)` paths on Linux in seconds. It is not the MSVC toolchain CI uses, so it proves types and not linking, which is enough to catch the whole class of "this half was never compiled".
6. **The gate, written out, because a weaker one locally is a red CI.** Clippy exits 0 on a warning unless it is told not to, so `cargo clippy --workspace --all-targets` passing locally says nothing about `cargo clippy --workspace --all-targets -- -D warnings`, which is what CI runs. That gap is what put `clippy::too_many_arguments` on a pull request's CI and not on the machine the code was written on, on 2026-09-04. The whole local gate, **in this order, which is not negotiable**: `cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, the same clippy line with `--target x86_64-pc-windows-gnu`, `cargo test --workspace --no-fail-fast`, `pnpm lint`, `pnpm format:check`, `scripts/check-vocabulary.sh`, then `pnpm e2e:build`, and only then the E2E battery.

  **The order is the point, and getting it wrong makes the battery lie.** `tauri build` compiles with `tauri/custom-protocol`, which is what serves the bundled `dist/`; a plain `cargo build` or `cargo test` rewrites `target/debug/sublore` without it, and that binary loads `http://localhost:1420` instead. With no Vite server there, the window opens on a blank page: the app starts, the frontend never mounts, and every spec dies waiting for an element that is never drawn. `e2e/README.md` has said so since M0.5. On 2026-09-04 a gate run was started while a battery was still going and swapped the binary under its last two specs, and three reruns afterwards all failed for the same reason on a binary nobody had rebuilt. Nothing that compiles Rust may run during a battery or after the build that a battery is about to use.
7. Review pipeline: significant changes go through a review pass before being presented as complete; findings are fixed or explicitly acknowledged in the PR description.
8. **A neutral CodeQL check on a fresh pull request is the rust analysis still running, not a broken configuration.** For the first few minutes of every pull request the "Code scanning results / CodeQL" check sits neutral, titled `1 configuration not found`, reporting that `/language:rust` was not among the configurations present on `refs/heads/main`. All three are present on `main` and nothing is misconfigured: the three languages analyse in parallel and upload separately, GitHub evaluates the check when the first upload arrives, and it then updates that same check in place as the others land. Measured on run 33653783733: uploads at 16:15:57, 16:16:13 and 16:19:25, a check created neutral at 16:15:56, and "No new alerts in code changed by this pull request" once the last one arrived, with its timestamps unchanged. The wait is the rust extractor reading the whole dependency graph before it reaches any of Sublore's own code, and it has measured between three and six minutes. Neutral while the rest of the run is still going is that wait; neutral after the whole run has finished is worth opening. It is left alone deliberately: every way of making the three uploads land together either delays all of them to the slowest language or loses a language's results when its job fails, and the scan is not made quieter or later than it is to change the colour of a badge.
...
- **Every behavioural verdict carries its platform.** Write "verified on Linux", never a bare "verified". Under the 2026-09-04 ruling this is the shipping condition rather than a stage on the way to something else, so it does not expire when v1.0 is tagged. A verdict with no platform on it reads as a claim about both, and that claim is false today.
